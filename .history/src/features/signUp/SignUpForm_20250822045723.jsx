import React, { useEffect } from "react";
import Button from "../../ui/Button";
import { Link, useNavigate } from "react-router-dom";
import { useForm } from "react-hook-form";
import InputError from "../../ui/InputError";
import useSignUp from "./useSignUp";
import useCurrentUser from "../login/useCurrentUser";

export default function SignUpForm() {
  const navigate = useNavigate() 
  const { register, handleSubmit, formState } = useForm();
  const { signUpMutate, isPending } = useSignUp();
  const {isAuth} = useCurrentUser()
  function successSubmit({ fullName, email, address, city, phone, password }) {
    signUpMutate({ fullName, email, address, city, phone, password });
  }
  useEffect(() => {
    if (isAuth) navigate("/menu")
  } , [])
  return (
    <form
      className="font-Vazirmatn flex flex-col gap-y-2"
      onSubmit={handleSubmit(successSubmit)}
    >
      <div className="flex flex-col">
        <label htmlFor="fullName" className="mb-1 text-center">
          نام و نام خانوادگی
        </label>
        <input
          {...register("fullName", { required: "لطفا نام خود را وارد کنید" })}
          id="fullName"
          type="text"
          className="bg-yellow-300 rounded-full outline-yellow-500 text-gray-500 text-center"
        />
        {formState.errors.fullName && (
          <InputError>{formState?.errors?.fullName?.message}</InputError>
        )}
      </div>
      <div className="flex flex-col">
        <label htmlFor="email" className="mb-1 text-center">
          ایمیل خود را وارد کنید
        </label>
        <input
          {...register("email", {
            pattern: {
              value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
              message: "لطفا ایمیل صحیح وارد کنید",
            },
            required: "لطفا ایمیل خود را وارد کنید",
          })}
          id="email"
          type="email"
          className="bg-yellow-300 rounded-full outline-yellow-500 text-gray-500 text-center"
        />
        {formState.errors.email && (
          <InputError>{formState?.errors?.email?.message}</InputError>
        )}
      </div>
      <div className="flex flex-col">
        <label className="mb-1 text-center" htmlFor="password">
          رمز عبور خود را انتخاب کنید
        </label>
        <input
          {...register("password", {
            minLength: {
              value: 8,
              message: "رمز عبور باید حداقل شامل ۸ کاراکتر باشد",
            },
            required: "لطفا رمز خود را وارد کنید",
          })}
          type="password"
          id="password"
          className="bg-yellow-300 rounded-full outline-yellow-500 text-gray-500 text-center"
        />
        {formState.errors.password && (
          <InputError>{formState?.errors?.password?.message}</InputError>
        )}
      </div>

      <div className="flex flex-col">
        <label className="mb-1 text-center" htmlFor="phone">
          شماره تلفن
        </label>
        <input
          {...register("phone", {
            required: "لطفا شماره خود را وارد کنید",
            minLength: {
              value: 11,
              message: "لطفا شماره صحیح را وارد کنید",
            },
          })}
          id="phone"
          type="number"
          className="bg-yellow-300 rounded-full outline-yellow-500 text-gray-500 text-center"
        />
        {formState.errors.phone && (
          <InputError>{formState?.errors?.phone?.message}</InputError>
        )}
      </div>

      <div className="flex flex-col">
        <label className="mb-1 text-center" htmlFor="city">
          استان خود را استان کنید
        </label>
        <input
          {...register("city", { required: "لطفا شهر خود را وارد کنید" })}
          id="city"
          type="text"
          className="bg-yellow-300 rounded-full outline-yellow-500 text-gray-500 text-center"
        />
        {formState.errors.city && (
          <InputError>{formState?.errors?.city?.message}</InputError>
        )}
      </div>

      <div className="flex flex-col">
        <label className="mb-1 text-center" htmlFor="address">
          ادرس خود را وارد کنید
        </label>
        <textarea
          {...register("address", { required: "لطفا ادرس خود را وارد کنید" })}
          id="address"
          className="bg-yellow-300 resize-none text-right outline-yellow-500 text-gray-500"
        />
        {formState.errors.address && (
          <InputError>{formState?.errors?.address?.message}</InputError>
        )}
      </div>

      <button
        type="submit"
        disabled={isPending}
        className="bg-yellow-400 cursor-pointer rounded-full mt-4  px-6 py-2"
      >
        ثبت نام
      </button>
      <Link className="text-right" to={"/login"}>
        قبلا ثبت نام کرده اید ؟ ورود
      </Link>
    </form>
  );
}
