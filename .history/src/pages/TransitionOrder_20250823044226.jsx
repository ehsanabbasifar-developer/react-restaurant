...
export default function TransitionOrder() {
  ...
  const { data, error , isPending} = useTransitionOrder(transitionId);
  
  return (
    <AppLayout>
      <div className="flex flex-col font-Vazirmatn gap-y-5 p-3 mt-9 md:w-3/4 md:mx-auto">
        <TransitionHeader transitionId={transitionId} />
        <TransitionDetails isLoading = {isPending} data={data} />
      </div>
    </AppLayout>
  );
}
